...
export default function AddFuelModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedLiters) || parsedLiters <= 0) {
      setError('Volume bahan bakar harus lebih besar dari 0 Liter!');
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        setError('Gagal menyimpan catatan bahan bakar ke database.');
      }
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Motor Aktif</label>
              <input
                type="text"
            ...
            <div className="form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Tanggal Isi</label>
                <input
                  type="date"
                  className="form-control"
                  value={date}
            ...
            <div className="form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Volume (Liter)</label>
                <input
                  type="number"
                  step="0.01"
                  className="form-control"
              ...
              <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Total Biaya (Rp)</label>
                <input
                  type="number"
                  className="form-control"
            ...
            {liters && price && !isNaN(parseFloat(liters)) && !isNaN(parseInt(price)) && (
              <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.01)', padding: '0.65rem 1rem', borderRadius: 'var(--radius-sm)', border: '1px dashed var(--border-color)', textAlign: 'center' }}>
                Estimasi Harga: <strong>Rp {Math.round(parseInt(price) / parseFloat(liters)).toLocaleString('id-ID')} / Liter</strong>
              </div>
            )}
          </div>
