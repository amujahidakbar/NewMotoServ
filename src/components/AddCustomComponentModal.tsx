...
export default function AddCustomComponentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedInterval) || parsedInterval <= 100) {
      setError('Service intervals must be greater than 100 KM!');
      return;
    }

    if (isNaN(parsedLastService) || parsedLastService < 0) {
      setError('Invalid last service odometer!');
      return;
    }

    if (Object.keys(activeMotor.intervals || {}).some(comp => comp.trim().toLowerCase() === name.toLowerCase())) {
      setError('A component with this name is already registered for your motorcycle!');
      return;
    }
    ...
    <div className="modal-backdrop open" id="modal-add-custom-component" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.7)', zIndex: 100 }}>
      <div className="modal-dialog" style={{ maxWidth: '400px', width: '90%', margin: '0 auto', background: 'var(--bg-surface-solid)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-lg)' }}>
        <div className="modal-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border-color)' }}>
          <h3 style={{ fontSize: '1.15rem', fontWeight: 600, color: 'var(--text-primary)' }}>Add New Component</h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ background: 'rgba(255, 255, 255, 0.01)', padding: '0.75rem 1rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
              Adding custom tracking component for motorcycle: <strong>{activeMotor.name}</strong>
            </div>

            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Component Name</label>
              <input
                type="text"
            ...
            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Service Interval (KM)</label>
              <input
                type="number"
                className="form-control"
                ...
                style={{ width: '100%', padding: '0.65rem 1rem', fontSize: '0.9rem', borderRadius: 'var(--radius-sm)' }}
              />
              <small style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Frequency of component replacement (e.g. replace every 3,000 KM).</small>
            </div>

            <div className="form-group" style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <label style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 500 }}>Last Service Odometer (KM)</label>
              <input
                type="number"
                className="form-control"
                ...
                style={{ width: '100%', padding: '0.65rem 1rem', fontSize: '0.9rem', borderRadius: 'var(--radius-sm)' }}
              />
              <small style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>Odometer reading when this component was last replaced. If this exceeds your motorcycle's current odometer, the motorcycle's odometer will be adjusted automatically.</small>
            </div>
          </div>
